...
def cut_docs(config):
    ...
    if os.path.isfile(cut_doc_file) and os.path.isfile(cut_doc_file_trec) and os.path.isfile(cut_doc_file_bert) and "cut_docs" not in config.force_steps:
        logging.info("Already found cut documents at %s. Skipping", cut_doc_file)
        return
    with open(cut_doc_file, 'w') as outf, open(cut_doc_file_trec, 'w') as outf_trec, open(cut_doc_file_bert, 'w') as outf_bert:
